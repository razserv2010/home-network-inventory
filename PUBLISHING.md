...
כדי להפעיל עדכון בלחיצה על הודעת גרסה חדשה בהתקנה קיימת, אחרי `git pull` מריצים פעם אחת `bash setup-web-upgrade.sh`. בהתקנה חדשה `install.sh` מגדיר זאת אוטומטית. עדכונים עתידיים דרך הממשק אינם משנים את הפורט. הפעולה אינה מוגנת בסיסמה, ולכן כל מי שמסוגל להגיע לממשק יכול להפעיל עדכון.
