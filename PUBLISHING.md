...
3. לחץ **Commit changes**. בדף הראשי אמורים להופיע `README.md`, `LICENSE`, `app.py`, `install.sh`, `UPGRADE`, `setup-web-upgrade.sh`, `upgrade-web.sh`, `requirements.txt`, `templates/index.html` ותיקיית `static` עם הסמל וה־manifest.
...
כדי להפעיל את הכפתור **עדכן עכשיו** בהתקנה קיימת, אחרי `git pull` מריצים פעם אחת `bash setup-web-upgrade.sh`. בהתקנה חדשה `install.sh` מגדיר אותו אוטומטית. עדכונים עתידיים דרך הכפתור אינם משנים את הפורט. הכפתור אינו מוגן בסיסמה, ולכן כל מי שמסוגל להגיע לממשק יכול להפעיל עדכון.
